...
public abstract class PlayerPathNavigate {
	protected EntityPlayerSP theEntity;
	...
	protected boolean shouldTrimToMinimal = true;

	public PlayerPathNavigate(EntityPlayerSP p_i1671_1_, World worldIn, PlayerMoveHelper playerMoveHelper) {
	...
	public final PathEntity getPathToXYZ(double p_75488_1_, double p_75488_3_, double p_75488_5_) {
		return this.func_179680_a(new BlockPos(MathHelper.floor_double(p_75488_1_), (int) p_75488_3_,
	 ...
	 * y, z, speed
	 */
	public boolean tryMoveToXYZ(double destX, double destY, double destZ, double speed) {
		PathEntity pathentity = this.getPathToXYZ((double) MathHelper.floor_double(destX), (double) ((int) destY),
				(double) MathHelper.floor_double(destZ));
		return this.setPath(pathentity, speed);
	}

	public boolean tryMoveToXYZ(BlockPos destination) {
		return this.tryMoveToXYZ(destination.getX() + 0.5D, destination.getY(), destination.getZ() + 0.5D, 1.0F);
	}
	...
	public void onUpdateNavigation() {
		...
		if (!this.noPath()) {
			...
			if (!this.noPath()) {
				...
				if (vec3 != null) {
					pmh.setMoveTo(vec3.xCoord, vec3.yCoord, vec3.zCoord, this.speed);
				}
			}
		}
	}
	...
	protected void pathFollow() {
		...
		// TODO test
		if (!shouldTrimToMinimal) {
			i = this.currentPath.getCurrentPathIndex() + 1;
		}
		...
		if (shouldTrimToMinimal) {
			...
			for (int i1 = i - 1; i1 >= this.currentPath.getCurrentPathIndex(); --i1) {
				if (this.isDirectPathBetweenPoints(vec3, this.currentPath.getVectorFromIndex(this.theEntity, i1), k, j1,
						l)) {
	...
	protected void func_179677_a(Vec3 p_179677_1_) {
		if (this.totalTicks - this.ticksAtLastPos > 100) {
			if (p_179677_1_.squareDistanceTo(this.lastPosCheck) < 2.25D) {
				System.out.println("func_179677_a()");
	...
	public void clearPathEntity() {
		System.out.println("clearPathEntity()");
		this.currentPath = null;
	}
	 ...
	 */
	protected abstract boolean isDirectPathBetweenPoints(Vec3 p_75493_1_, Vec3 p_75493_2_, int p_75493_3_,
			int p_75493_4_, int p_75493_5_);
}